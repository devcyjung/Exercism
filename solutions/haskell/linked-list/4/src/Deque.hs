...
push Deque { dqSize = dqSizeRef
  ...
  case maybeLast of
    ...
    Just Node { nPost = oldLastNodePostRef } -> do
      writeIORef dqLastRef (Just newNode)
      writeIORef oldLastNodePostRef (Just newNode)

unshift :: Deque a -> a -> IO ()
unshift Deque { dqSize = dqSizeRef
              , dqFirst = dqFirstRef
              , dqLast = dqLastRef
              } x = do
  modifyIORef' dqSizeRef (+ 1)
  ...
  case maybeFirst of
    Nothing -> do
      writeIORef dqFirstRef (Just newNode)
      writeIORef dqLastRef (Just newNode)
    Just Node { nPrev = oldFirstNodePrevRef } -> do
      writeIORef dqFirstRef (Just newNode)
      writeIORef oldFirstNodePrevRef (Just newNode)

shift :: Deque a -> IO (Maybe a)
shift Deque { dqSize = dqSizeRef
            , dqFirst = dqFirstRef
            , dqLast = dqLastRef
            } = do
  maybeFirst <- readIORef dqFirstRef
