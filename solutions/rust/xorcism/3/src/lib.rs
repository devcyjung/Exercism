...
impl<'a> Xorcism<'a> {
    ...
    /// Returns reader stream adaptor which munges the buf output with xorcism's key
    pub fn reader(self, reader: impl Read) -> impl Read {
        XorcismReader{ xor: self, reader }
    }

    /// Returns writer stream adaptor which munges the buf input with xorcism's key
...
impl<R> Read for XorcismReader<'_, R>
where
    R: Read + ?Sized,
{
...
impl<W> Write for XorcismWriter<'_, W>
where
    W: Write + ?Sized,
{
