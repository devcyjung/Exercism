...
inline constexpr
bool is_valid_char(char ch) noexcept {
...
inline constexpr
std::optional<int> safe_compute(std::string_view strand1, std::string_view strand2) noexcept {
    if (strand1.size() != strand2.size()) {
        return {};
    }
    int distance = 0;
    for (size_t i = 0; i < strand1.size(); ++i) {
        char ch1 = to_upper(strand1[i]);
        char ch2 = to_upper(strand2[i]);
