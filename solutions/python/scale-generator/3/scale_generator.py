...
class Scale:
    ...
    def interval(self: 'Scale', intervals: str):
        return self.__generate_sequence(accumulate(chain((0,), map(Scale.__convert_interval, intervals))))

    def __generate_sequence(self: 'Scale', iterable: Iterable[int]):
        return [self.__series[(self.__base_idx + delta) % len(self.__series)] for delta in iterable]
