        ...
        return Observable.merge(words, letters).map(s -> {
            if (s.length() == 1) {
                ...
                if (secret.contains(s)) {
                    guess = Stream.concat(guess.stream(), Stream.of(s)).collect(Collectors.toSet());
                    status = discovered.contains("_") ? Status.PLAYING : Status.WIN;
                } else {
                    misses = Stream.concat(misses.stream(), Stream.of(s)).collect(Collectors.toSet());
                    parts = Stream.concat(parts.stream(), Stream.of(PARTS[parts.size()])).collect(Collectors.toList());
