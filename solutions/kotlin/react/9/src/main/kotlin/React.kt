...
class Reactor<T>() {
    ...
    open inner class InputCell(initialValue: T) {
        private val onChangeCallbacks: MutableSet<Callback<T>> = mutableSetOf()
        ...
        fun addCallback(callback: Callback<T>): Subscription = Subscription {
            onChangeCallbacks.remove(callback)
        }.also { onChangeCallbacks.add(callback) }
    }

    inner class ComputeCell(vararg val cells: InputCell, val computeFn: Compute<T>) :
